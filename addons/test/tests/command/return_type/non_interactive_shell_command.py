...
class TestTestCommandReturnTypeNonInteractiveShellCommand(AbstractTestCase):
    def test_non_interactive_shell_command(self):
        self.for_each_render_mode(self._test_non_interactive_shell_command, {
            KERNEL_RENDER_MODE_NONE: None,
            KERNEL_RENDER_MODE_JSON: json.dumps({"value": "NON_INTERACTIVE_SHELL_COMMAND_RESPONSE"}),
            KERNEL_RENDER_MODE_TERMINAL: 'NON_INTERACTIVE_SHELL_COMMAND_RESPONSE',
        })

    def _test_non_interactive_shell_command(self, render_mode):
