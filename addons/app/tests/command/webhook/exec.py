...
class TestAppCommandWebhookExec(AbstractWebhookTestCase):
    def test_exec(self) -> None:
        ...
        # Async hook
        http_response = self.request_listener(
            "/webhook/app/test/wex/webhook/test-waiting", check_code=None, wait=2
        )
        ...
        self.assertTrue(" test-file" in lines[0][1])
        self.assertEqual(lines[0][3], "TEST_EXECUTION_ORDER")
        self.assertEqual(lines[0][4], test_arg)
