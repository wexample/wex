...
@option("--service", "-s", type=str, required=True, help="Service name")
@option(
    "--install-config",
    "-ic",
    type=bool,
...
def app_service_install_merge_dir(
    ...
    if os.path.isdir(abs_path):
        ...
        dest_file = os.path.join(app_dir, APP_DIR_APP_DATA, current_item)
        ...
        if basename == ".gitignore.sample":
            if install_git:
                ...
                # Override file name.
                dest_file = os.path.join(app_dir, APP_DIR_APP_DATA, ".gitignore")

                file_create_parent_and_touch(dest_file)

                file_merge_new_lines(abs_path, dest_file)
            ...
            if install_docker:
                ...
                if "services" in extra_compose:
                    ...
                    for service in extra_compose["services"]:
                        extra_services[f"{app_name}_{service}"] = extra_compose[
                            "services"
                        ][service]
