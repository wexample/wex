...
def app__remote__push_receive(
    ...
    def _app__remote__push_receive_copy_to_destination(
        ...
        file_create_directories_and_copy(temp_path_resolved, destination_path)

    def _app__remote__push_receive() -> None:
        manager.get_directory().process_schema_recursive(
            _app__remote__push_receive_copy_to_destination
        )

    manager.exec_in_app_workdir(app_dir, _app__remote__push_receive)
