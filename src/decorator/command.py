...
def command(*decorator_args: Args, **decorator_kwargs: Kwargs) -> Callable[..., ScriptCommand]:
    if "help" not in decorator_kwargs:
        raise ValueError(
            "The 'help' argument is required for the custom command decorator."
        )

    def decorator(f: AnyCallable, script_command_class: type = ScriptCommand) -> ScriptCommand:
