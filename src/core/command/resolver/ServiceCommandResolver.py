...
class ServiceCommandResolver(AbstractCommandResolver):
    ...
    def build_path(self, request: CommandRequest, extension: str, subdir: Optional[str] = None) -> Optional[str]:
        ...
        return self.build_command_path(
            ...
            extension=extension,
            subdir=subdir,
            command_path=os.path.join(string_to_snake_case(request.match.group(2)),
    ...
    def autocomplete_suggest(self, cursor: int, search_split: []) -> str | None:
        ...
        if search_split[0] == COMMAND_CHAR_SERVICE:
            if cursor <= 1:
                if search_split[0] == COMMAND_CHAR_SERVICE:
                    commands = [
                        command for command in self.get_commands_registry().commands.keys()
                ...
                if search_split[2] == COMMAND_SEPARATOR_ADDON:
                    ...
                    commands = [
                        command[len(search_service):] for command in self.get_commands_registry().commands.keys()
