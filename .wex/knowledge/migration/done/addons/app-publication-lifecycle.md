# App Publication Lifecycle — Closed v6 lane
...
| **Application** | Syrtis API, manager | Serveur/environnement | Implémenté et validé |
...
**Principe :** c'est la commande qui se déclare, pas l'orchestrateur qui cherche. Les points d'ancrage sont les commandes réelles du pipeline (`app::app/publish`, `app::package/bump`, `app::file/rectify`...). Aucune convention de nommage à respecter.
...
| `version/new_write` | `app::file/rectify` |
...
- `app::app/publish` existe dans `wex-addon-app`
- le flux a été rejoué avec succès sur une vraie app de référence
- le premier `app::app/publish` successful a été confirmé en local
- le sujet n'est donc plus considéré comme bloquant pour la migration v6
