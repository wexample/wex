...
- [ ] Rendering is a transformation stage, not just variable substitution: translating
      ...
      written in English while the author works in French. Decided since: French is
      **mandatory and always built**, because an author who has to ask for it every time
      will stop asking; any other language is opt-in and the rectify pass does not chase
      it. Freshness keys off the **engrams** — `last_rev` per source already does exactly
      this job for the `english` agent, so a stale translation is a solved problem rather
      than a new cache to design. The translation stage reuses the existing
      filestate-delegates-content-to-an-agent mechanism, it is not a new kind of agent
- [x] **Build a markdown rendering into the repository**, for the reader without wex —
      ...
      languages join as sibling directories without a restructure.
      Two things it does not do yet: the lookup is `_knowledge()`, single-level, so a page
      inherited from a suite is invisible to it (the phase 5 bullet above); and a plain
      `.md` source is copied as prose, only `.md.j2` goes through Jinja
...
Python suite. **Reading it** now has two working surfaces out of the five mapped — the
generated README (case A) and the built pages (part of case F) — and remains the
bottleneck.

Phase 6 has started from its artefact rather than its commands: `built/en/` exists and
is committed, which already gives `AGENTS.md` something real to point at, and gives
`read` a corpus to serve instead of a cascade to re-resolve.

Next step: finish the **writing** side before opening `read`, so the axis closes rather
than staying half-open. Concretely, in order — take `en` out of the hard-coded tree (the
mandatory-language list in config, global with a per-app override, the tree looping over
it), which lands `built/fr/` still in English and touches no agent; then plug the
translation stage onto that render. The command families come after, and `AGENTS.md`
(case B) last, as already decided: it can only stop pointing at dead ends once there are
commands and an artefact to point at instead.
