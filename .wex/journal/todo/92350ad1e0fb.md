...
- [x] **Dropped**: extending the 4-level cascade to knowledge pages, so a page written
      once in a suite would be readable from every package. A page inherited that way
      materialises in each package's `built/<lang>/`, is translated once per package —
      the engram is keyed by workdir, nothing tells the translator two copies are the
      same text — and gets committed eighty times. The need was never duplication but
      addressing: the suite's pages already exist, once, at the right level; what is
      missing is that a reader inside a package has nothing telling him so. That is
      what `knowledge/read|search` answers, so phase 6 removes this need rather than
      depending on it
- [ ] Switch remaining `.md` fragments to `.j2` — no longer weighed against case C,
      which phase 6 settles
- [x] Exported `.md` files are **versioned generated artefacts**: committed like the
      README and `built/`, since a reader who clones without wex has nothing else. A
      diff on one of them is read as a documentation change, not as noise
- [x] `readme.sections` removed from `PACKAGES/.wex/config.yml`. The composer decides
      the order for every package it governs, and the key listed `suite-integration`
      twice — a duplicate nobody noticed, which is what a dead setting does. Suites
      without a composer fall back to discovery order
...
- [x] **Dropped**: extending `read` to the app's own knowledge. It was meant to serve a
      page by identifier in its built form, but `built/<lang>/` already puts that exact
      text in a flat file at a path the search answers with, and every reader — human or
      agent — can open a file. A command that only spells `Read` differently earns
      nothing. `app::knowledge/read` stays what it is, the reader of *service* knowledge,
      where there is no obvious path to open
- [x] Rendering is a transformation stage, not just variable substitution — first case
      shipped: translation. The mandatory-language list left the hard-coded tree for
      `knowledge.languages` in the suite config, and `built/<lang>/` loops over it.
      `KnowledgePageTranslationConfigValue` translates the *render*, not the file under
      `built/en/`, so the two directories have no ordering dependency; freshness keys off
      an engram holding the sha256 of that render, so a page already translated costs no
      model call. A workdir with no AI addon returns nothing and the language is dropped
      whole rather than filled with English
- [x] **Build a markdown rendering into the repository**, for the reader without wex —
      `.wex/knowledge/built/en/`, mirroring the source taxonomy, one standalone file per
      page, **committed**: it is the only branch of `AGENTS.md` that works offline, so it
      has to survive a clone. `KnowledgePageBuildConfigValue` renders through the same
      pipeline `{{ knowledge() }}` uses, so a page reads identically inlined into a README
      or opened on its own. `built/en/` from the start rather than `built/`, so other
      languages join as sibling directories without a restructure — which `fr` then did.
      One limit kept: a plain `.md` source is copied as prose, only `.md.j2` goes through
      Jinja

**Search (case G)** — `app::knowledge/search` for the documentation,
`app::sources/search|grep` for the code.

- [x] Answers "which package handles X" across a suite, which no other surface does:
      `app::knowledge/search --query <text>` climbs to the highest suite through
      `find_stack_root`, descends into every package through `collect_app_tree` — both
      moved out of `journal_tree.py`, which had them under a name that hid them — and
      greps each `built/<lang>/`. `--language` searches the translations, since they are
      renders of the same pages
- [x] **Answers at the page level**, not line by line: the path, how many matches it
      holds and the headings they fall under. Decided from the corpus — the median built
      page is 40 lines, so a broad query answered line by line costs more to read than
      the pages it points at, and the sections say which one to open without opening any.
      `--lines` gives the grep form, excerpts windowed around the term since a fifth of
      the corpus runs past 200 characters. Nothing found says so in a sentence, rather
      than answering with an empty string a caller cannot tell from a crash
- [x] **The asking app's pages come first**, whatever the scope — the ordering is the
      walk order, the caller simply leads the list. It is the preference an agent needs
      most: working inside a package, what that package says about a subject beats what
      seventy-six others say, without any of them being hidden
- [x] `--scope self|suite|stack` narrows the reach, `stack` being the default since
      reaching past the current app is the only thing the command adds over opening a
      file. `suite` means the packages an app holds when it is itself a suite, and the
      suite it belongs to otherwise. Page paths stay relative to the top of the stack in
      every scope, so one path always means one thing
- [x] Mechanism decided: **no index**. 150 pages and 7 000 lines across the whole stack,
      read once per call — an index would be a second thing to keep fresh for a corpus a
      live grep crosses in a blink
...
- [x] Exposed as an agent tool, which closes gap D: `AudienceTag.AGENT_SAFE` puts it on
      MCP, and the shared agent context now says to search the stack before concluding a
      subject is undocumented — an agent stands in one package and cannot otherwise learn
      the other seventy-six exist
...
Phases 1 to 5 and 7 complete for Python. The whole chain has run at scale: the 37
Python packages each have a rebuilt README, the three mandatory pages written — not one
left as a stub — and `built/en/` plus `built/fr/` committed. Which is the proof the
mechanism was built for, the composer having to hold across 37 repositories without a
file in any of them.

Read against the three axes: **writing it** is closed for Python, and so is **fixing
it**. **Reading it** has three working surfaces out of the five mapped — the generated
README (case A), the built pages in two languages (part of case F), and stack-wide search
(case G) — and is now the only front left.

What remains of phase 6 is `sources/search`, then `AGENTS.md` (case B) last, since it can
only stop pointing at dead ends once there are commands and an artefact to point at
instead. Then the non-Python repositories, once their language addons ship the templates
the Python one has.
