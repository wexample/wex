...
it**. **Reading it** has two working surfaces out of the five mapped — the generated
README (case A) and the built pages, in two languages (part of case F) — and is now the
only front left.

What remains is phase 6, in the order already decided: `knowledge/read`, then `search`,
then `AGENTS.md` (case B) last, since it can only stop pointing at dead ends once there
are commands and an artefact to point at instead. Then the non-Python repositories, once
their language addons ship the templates the Python one has.
