#!/usr/bin/env bash

sshExecArgs() {
 _ARGUMENTS=(
   [0]='dir d "Local root site directory" true'
   [1]='shell_script s "Command to execute from shell, relative to site directory" true'
 )
}

sshExec() {
  wex wexample::site/deployCredentials -d=${DIR}
  # Go do site path
  # Then execute script from _exec.sh tool of wexample.
  ssh -t -p${DEPLOY_PORT} ${DEPLOY_USER}@${DEPLOY_IPV4} "cd ${DEPLOY_PATH_ROOT} && echo "OK" && cat ${WEX_DIR_BASH_DEFAULT}_exec.sh && sudo bash ${WEX_DIR_BASH_DEFAULT}_exec.sh ${SHELL_SCRIPT}"
}
