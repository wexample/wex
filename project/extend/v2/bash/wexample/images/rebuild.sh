...
imagesRebuild() {
  ...
  if [[ ${DEPLOY} == true ]]; then
    ...
    if [ ! -z "${LSC+x}" ]; then
      DOCKER_USERNAME=$(wex var/get -n=DOCKER_USERNAME -ask="Docker username for wex images deployment")
      DOCKER_PASSWORD=$(wex var/get -n=DOCKER_PASSWORD -ask="Docker password" -p)
    else
